...
public class Isa2OwlService {
	...
	public String getInit() {
		...
		ConvertHandler handler = new ConvertHandler(serviceID);
	...
	public Response convertFile(@FormDataParam("serviceID") final String serviceID,
		...
		if (!isValidParameters(serviceID, convertedFile, fileDetail, handler)
			...
			formData.field("response", new MessagesBean(ResponseTypes.ERROR, 1, "Invalid Request")
			.toJSONString());
		...
		handler.init(convertedFile, CONVERT_PATH, filename, filesize);
		convertHandlers.put(serviceID, handler);
		...
		FormDataMultiPart formData = new FormDataMultiPart();
		formData.bodyPart(new FileDataBodyPart("owlfile", handler.getOwlFile()));
